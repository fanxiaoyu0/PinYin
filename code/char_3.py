...
def init(): # 初始化字频，条件频率矩阵，拼音汉字对照表,并将汉字与数字id一一对应起来
    ...
    pickle.dump(Pinyin2Char, open('../模型/字三元/拼音汉字对照表.pkl', 'wb'))  # 保存拼音汉字对照表
    EndTime = time.time()
...
def count(): # 统计字频,两字相邻频率,三字相邻频率
    ...
    for item in FileList: # 遍历语料库文件
        ...
        for line in TxtLines:
            ...
            if CountLine % 1000 == 0: # 每1000行输出一下进度
                print("现在训练到第", CountTxt, "/",len(FileList),"个文件的第", CountLine, "/", TotalLines, "行",
                      "三元词文件大小为：",sys.getsizeof(TripleFreq)/1024,"kB")
                # print(TripleFreq)
            sentences = [] # 这一行里的很多句子
            sentence = '' #某一个句子
            for c in line:
        ...
        print("--------------------------------------------------")
        print(TripleFreq['原'])
    pickle.dump(SingleFreq, open('../模型/字三元/1.2/字频.pkl', 'wb')) # 保存字频，下次启动就不用再算了
    pickle.dump(DoubleFreq, open('../模型/字三元/1.2/两字相邻频率.pkl', 'wb'))  # 保存条件频率矩阵
    pickle.dump(TripleFreq, open('../模型/字三元/1.2/三字相邻频率.pkl', 'wb'))  # 保存条件频率矩阵
    EndTime = time.time()
    print("count函数运行时间为:",  EndTime - StartTime, "s")

def viterbi(PinSentence, Pinyin2Char,SingleFreq, DoubleFreq ,TripleFreq):
                ...
                for PrevChar in Pin2Han[i-1].keys():# 不用Pinyin2Char[PinSentence[i-1]],考虑到上一层可能剪枝
                    ...
                    if c in DoubleFreq.keys():
                        if PrevChar in DoubleFreq[c].keys():
                            ...
                            temp = Pin2Han[i - 1][PrevChar] + math.log(4000000 * x / SingleFreq[PrevChar] + 0.0002* SingleFreq[c])
                        else:
                            temp = Pin2Han[i - 1][PrevChar] + math.log(0.0001 * SingleFreq[c])
                    else:
...
def predict():
    ...
    OutFile = "test/OUT-3.5.txt"#sys.argv[2]  #"test/OUT-1.1.txt"  默认输出到data文件夹下
    Pinyin2Char = pickle.load(open('../模型/字三元/拼音汉字对照表.pkl', 'rb'))  # 载入拼音汉字对照表
    SingleFreq = pickle.load(open('../模型/字四元/1.4/字频.pkl', 'rb'))  # 载入字频
    DoubleFreq = pickle.load(open('../模型/字四元/1.4/两字相邻频率.pkl', 'rb'))  # 载入条件频率矩阵
    TripleFreq = pickle.load(open('../模型/字四元/1.4/三字相邻频率.pkl', 'rb'))  # 载入条件频率矩阵
    ...
    for line in IN.readlines():
        ...
        HanSentence=viterbi(PinSentence, Pinyin2Char, SingleFreq, DoubleFreq, TripleFreq) # 使用viterbi算法得到预测的句子
...
def check(): # 计算模型的准确度，包括字准确度和句准确度
    OUT = open('../data/test/OUT-3.5.txt',encoding="utf-8")
